...
### Bulk culture microscopy and growth analysis, Figure 3
...
#### Make the plot for Supplementary Figure 3
```{r}
...
### Aphidocholin microscopy analysis, Figure 4
...
cohen.d(data.aph.u.t1$oqn.area, data.aph.u.t3$oqn.area, paired = FALSE)
...
cohen.d(data.aph.scomp.t1$oqn.area, data.aph.scomp.t3$oqn.area, paired = FALSE)

cohen.d(data.aph.scomp.t1$oqn.area, data.aph.u.t1$oqn.area)

```

### Single cell time lapse imaging analysis, Figure 5
...
#### Calculate cell area doubling time and plot distributions (Figure 5A)
```{r}
...
#### Calculate stats for doubling time (text corresponding to figure 5A)
```{r}
...
#### Plot the division ratio histogram and raw data (lower subpanel 5B)
```{r} 
...
### Nanostring gene expression analysis, Figure 7
...
#### Plot all the genes, Supplementary Figure 7A
```{r}
...
##### Plot the data for main text Figure 7A
```{r}
...
pdf("fig7A.pdf" , width = 4, height = 4)
ggplot(cdr.plot) +
  geom_point(aes(x = Category, y = log.expr, color = Strain), 
              size = 1, alpha = 0.5) +
  geom_smooth(aes(x = Category, y = log.expr, 
...
##### plot the data to make Supplementary Figure 7B
```{r}
...
##### Write output files for supplementary table 7
```{r}
#save the list of genes differentially expressed in response to growth in the parent strain. Also make data frame for plotting for figure. The facet wrap labels for these 10 signif genes were highlighted manually in Supplementary Fig 7A.
...
### ChIP-seq visualization with trackViewer (Figure 2).
...
#### make the plot, supplementary Figure S2A
```{r}
...
#### Make the plot ready for main text, Figure 2
```{r}
...
#### Make the plots for supplementary Figure S2B
```{r}
#make new tracks that allow overplotting
log3<-log.pks3
